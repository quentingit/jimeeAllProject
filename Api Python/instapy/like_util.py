...
def get_user_infos(browser, id_insta, insta_username, firstentry):

    #images = browser.find_elements_by_tag_name('img')
    #for image in images:
    #    print(image.get_attribute('src'))

    insta_username = insta_username.replace(" ", "")


    print("ON REFRESH LES INFOS EN BDD : pour la table instauser et stats!")
    ...
    try:
        ...
        while (i < 5):
            try:
                print("pseudo")
                # on recupere l'avatar
                WebDriverWait(browser, 0).until(EC.presence_of_element_located((By.XPATH, "//h1")))
                pseudo = browser.find_element_by_xpath("//h1").text
                print("PSEUDO EST")
                print(pseudo)
                break
            except:
        ...
        #IL FAUDRA REGARDER LES "K" pour 100 000
        #IL FAUDRA REGARDER LES "K"
        #IL FAUDRA REGARDER LES "K"
        #IL FAUDRA REGARDER LES "K"
        #IL FAUDRA REGARDER LES "K"
        ...
        #IF : on check si le nom d'utilisateur est le bon
        if pseudo == insta_username:

            #on recupere le nb post, followers et followings
            #print("ON AFFICHE TOUTE LA PAGE  ==============================>")
            ...
            while (i < 1):
                ...
                try:
                    print("1ere adresse")
                    # on recupere l'avatar
                    WebDriverWait(browser, 0).until(EC.presence_of_element_located((By.XPATH, "//header/div/div/span/img")))
                    avatar = browser.find_element_by_xpath("//header/div/div/span/img").get_attribute("src")
                    print("AVATAR EST")
                    print(avatar)
                    break
                except :
                    try:
                        print("test 2eme adresse")
                        # on recupere l'avatar
                        WebDriverWait(browser, 0).until(EC.presence_of_element_located((By.XPATH, "//header/div/div/div/button/img")))
                        avatar = browser.find_element_by_xpath("//header/div/div/div/button/img").get_attribute("src")
                        print("AVATAR EST")
                        print(avatar)
                        print("--------------------")
                        break
                    except:
                        print("ca deconne donc faut trouver")
                        i = i + 1
            ...
            while(i<1):
                try:
                    ...
                    print("1ere nbpost")
                    WebDriverWait(browser, 0).until(EC.presence_of_element_located((By.XPATH, "//header/section/ul/li[1]/span/span")))
                    p1 = browser.find_element_by_xpath("//header/section/ul/li[1]/span/span").text
                    posts = ''.join(c for c in p1 if c.isdigit())
                    print("NB POSTS EST")
                    print(posts)
                    break
                except:
                    try:
                        print("2eme nb post")
                        # on check si c'est bon
                        WebDriverWait(browser, 0).until(EC.presence_of_element_located((By.XPATH, "//header/section/ul/li[1]/a/span")))
                        p1 = browser.find_element_by_xpath("//header/section/ul/li[1]/a/span").text
                        posts = ''.join(c for c in p1 if c.isdigit())
                        print("NB POSTS EST")
                        print(posts)
                        break
            ...
            try:
                f1 = browser.find_element_by_xpath("//header/section/ul/li[2]/a/span").text
                followers = ''.join(c for c in f1 if c.isdigit())
                print("NB FOLLOWERS EST")
                print(followers)
            except:
                pass

            try:
                f2 = browser.find_element_by_xpath("//header/section/ul/li[3]/a/span").text
                followings = ''.join(c for c in f2 if c.isdigit())
                print("NB FOLLOWINGS EST")
                print(followings)
            except:
                pass
            ...
            #UPDATE DES INFOS DANS DANS LA TABLE  : instausers
            cur = cursor()
            sql_insert_query = """ UPDATE  instausers set avatar = %s, n_posts= %s,n_followers=%s, n_followings=%s where instauser_id=%s"""

            print("ID INSTACONFIG")
            print(id_insta)

            val = (avatar, posts, followers, followings,id_insta)
            result = cur.execute(sql_insert_query, val)



            #POUR LES STATS INFOS DE L'UTILISATEUR

            dateNow = datetime.date.today()
            fisrtDate= dateNow - datetime.timedelta(days=1)
            # regarder si pour la date on a les infos dans la table stats
            cur = cursor()
            sql = """SELECT COUNT(id_statsinfos) FROM statsInfos WHERE id_userinsta=%s and log_date=%s """
            val = (id_insta, dateNow)
            cur.execute(sql, val)
            ...
            if myresult[0]==0 :
                ...
                if firstentry==1 :
                    cur = cursor()
                    sql_insert_query = """ INSERT INTO `statsInfos`(`id_userinsta`,`n_posts`,`n_followers`,`n_followings`,`log_date` ) VALUES (%s, %s, %s,%s, %s)"""
                    val = (id_insta, posts, followers, followings, fisrtDate)
                    result = cur.execute(sql_insert_query, val)
                #INSERT : si pas d'infos pour la date du jour
                cur = cursor()
                sql_insert_query = """ INSERT INTO `statsInfos`(`id_userinsta`,`n_posts`,`n_followers`,`n_followings`,`log_date` ) VALUES (%s, %s, %s,%s, %s)"""
                val = (id_insta,posts,followers,followings, dateNow)
                result = cur.execute(sql_insert_query, val)
                print("INSERTION INFOS STATS=======> ")
            else:
                # UPDATE : si infos pour la date du jour
                cur = cursor()
                sql_insert_query = """ UPDATE  statsInfos set  n_posts= %s,n_followers=%s, n_followings=%s where id_userinsta=%s  and log_date=%s """
                val = (posts, followers, followings,id_insta, dateNow)
                result = cur.execute(sql_insert_query, val)
                print("UPDATE INFOS STATS=======> ")

            return True, "success"

        else:
            return False, "not find user"
    except:
        print("Couldn't find it!")



def like_image(browser, username, userinsta, blacklist, logger, logfolder):
    ...
    if len(like_elem) == 1:
        ...
        if len(liked_elem) == 1:
            ...
            print('--->user db :', username)
            #for loggingdbb
            cur=cursor()
            request = "image Liked"

            sql_insert_query = """ INSERT INTO `logs`(`type`,`user`,`instausers_id`) VALUES (%s, %s, %s)"""
            val = ("liked", username, userinsta)
