...
def follow_user(browser,id_insta, track, login, user_name, button, blacklist, logger,
                logfolder):
...
def unfollow_user(browser, id_insta, track, username, person, person_id, button,
    ...
    dateNow = datetime.datetime.now()
    sql_insert_query = """ INSERT INTO `logs`(`type`,`user`,`instausers_id` ) VALUES (%s, %s, %s)"""
    val = ("unfollow", username, id_insta)
    result = cur.execute(sql_insert_query, val)
